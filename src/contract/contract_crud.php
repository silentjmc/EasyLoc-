...
class ContractCrud {
     ...
     */
    public function getOngoingRentalsbyCustomerUid(string $customerUid): array {
     ...
     * 
     */
    public function getTotalOverdueRentalsBetweenDates(DateTime $start, DateTime $end): int {
     ...
     * 2.3
     */
    public function getAverageOverdueRentalsByCustomer(): array {
        $sql = "SELECT customer_uid, (SUM(CASE WHEN (returning_datetime IS NULL AND loc_end_datetime < NOW() - INTERVAL 1 HOUR) 
              OR (returning_datetime IS NOT NULL AND returning_datetime > loc_end_datetime + INTERVAL 1 HOUR) THEN 1 ELSE 0 END
    ...
    public function getAverageTimeOverdueByVehicle(): array {
        $sql = "SELECT vehicle_uid, AVG(TIMESTAMPDIFF(MINUTE, loc_end_datetime, returning_datetime)) AS average_time_overdue 
